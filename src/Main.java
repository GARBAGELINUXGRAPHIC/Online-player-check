...
public class Main extends Plugin implements Listener{
	public void onEnable() {
		...
		BungeeCord.getInstance().getLogger().info("Read OCB config...");
		Utils.readcfg();
		if(Utils.cfgstats == false) {
			BungeeCord.getInstance().getLogger().warning("&4OCB FATAL ERROR:FAIL TO READ CONFIG");
			sleep(3*1000);
		} else {
			BungeeCord.getInstance().getLogger().info(Utils.online_graph_ctx_font_1 + " | " + Utils.online_graph_ctx_fillStyle_1 + " | "
					+ Utils.online_graph_ctx_fillStyle_2 + " | " + Utils.online_graph_ctx_font_2 + " | " + Utils.online_graph_ctx_strokeStyle_2);
